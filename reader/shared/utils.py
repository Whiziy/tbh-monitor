...
class _Tee:
    """Writes to multiple streams (console + file). Tolerant of I/O errors."""
...
def tee_stdio(log_path, max_bytes=5_000_000):
    """Mirrors stdout+stderr to a file (besides the console)."""
    try:
...
def init_diag_log(log_path, max_bytes=5_000_000):
    """Opens the infra log (reader-diag.log), separate from meter.log."""
    global _DIAG
    try:
        os.makedirs(os.path.dirname(log_path) or ".", exist_ok=True)
...
def diag(msg):
    """Appends a [HH:MM:SS] line to the infra log. No-op if not initialized."""
    f = _DIAG
    if f is None:
        return
    try:
